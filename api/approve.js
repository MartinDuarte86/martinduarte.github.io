...
// Seguridad:
//   - JWT firmado con APPROVAL_SECRET, expiración 48h
//   - One-time-use via SET NX atómico (TOCTOU-safe)
//   - Acepta GET (links de email) y POST (uso programático)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const msg = err.name === 'TokenExpiredError'
      ? 'Este link expiró (48 horas). Solicitá un nuevo link.'
  ...
  if (!isFirstUse) {
    return res.status(410).send(errorPage('Link ya utilizado', 'Este link de aprobación ya fue usado.'));
  }
  ...
  try {
    ...
    // ── 4. Recuperar HTML del diseño elegido ────────────────────────────────
    ...
    if (!dsn?.html_preview) {
      return res.status(404).send(errorPage(
        ...
        'No se encontró el HTML del diseño. Puede haber expirado.'
      ));
    }

    // ── 5. Disparar GitHub Actions ──────────────────────────────────────────
    ...
    const ghResponse = await fetch(
      `https://api.github.com/repos/${GH_OWNER}/${GH_REPO}/actions/workflows/deploy-landing.yml/dispatches`,
      {
        ...
        body: JSON.stringify({ ref: 'main', inputs: { cliente_slug: slug, html_content: htmlBase64 } }),
      }
    );

    if (!ghResponse.ok) {
      console.error('[approve] GitHub Actions error:', ghResponse.status);
    }

    const landingUrl = `${BASE_URL}/clientes/${slug}/`;
    return res.status(200).send(successPage(
      '✅ Deploy aprobado',
      `La landing de <strong>${escapeHtml(nombre_marca)}</strong> fue aprobada. GitHub Actions está desplegando.`,
      landingUrl
    ));

  } catch (err) {
    console.error('[approve] Error interno:', err.message);
    return res.status(500).send(errorPage('Error interno', 'Ocurrió un error al procesar la aprobación.'));
  }
}
