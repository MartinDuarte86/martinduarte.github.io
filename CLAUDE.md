...
- `save-client.js` / `save-session.js` / `get-session.js` — CRUD de clientes y sesiones contra Supabase/Redis. **Identidad de sesión (Fase 1):** `save-client.js action=create` emite una **cookie httpOnly firmada** (`_lib/session.js`, `SESSION_SECRET`) que ata el `session_id` a ese navegador; `get-session.js`/`save-session.js` derivan el `session_id` **de la cookie vía `requireSession`** (no del body/query) → cierra el IDOR / la mezcla de conversaciones. `action=reset` expira la cookie ("empezar de nuevo").
- `_lib/session.js` — emisión/verificación de la cookie de sesión (`issueSessionCookie`, `getSessionId`, `requireSession`, `clearSessionCookie`). El sid de la cookie es la única fuente de verdad de qué sesión se toca; los handlers ignoran cualquier `session_id` del cliente.
- `save-dsn.js` / `get-dsn-html.js` — persistencia y recuperación de design sets ("diseños anteriores"). `save-dsn.js` (POST) deriva `session_id` de la cookie y resuelve `client_id` server-side por lookup en `clients`.
...
**Gotcha de `client_id` (resuelto en Fase 1, vigilar regresión):** históricamente el browser generaba un UUID local para `client_id` y un desajuste con el `id` real de Supabase rompía la FK de `design_sets` en silencio. Desde la Fase 1, `save-dsn.js` **resuelve `client_id` server-side** por lookup en `clients` usando el `session_id` de la cookie — el front ya no manda `client_id`. No reintroducir el envío de `client_id` desde el browser.

**Seguridad de sesión (Fase 1):** la identidad es server-side vía cookie httpOnly firmada (ver `_lib/session.js` arriba). No volver a confiar en el `session_id` del body/query para autorizar acceso a datos de sesión, ni reintroducir la auto-reanudación silenciosa por `localStorage` en `validator.js` (metía a un usuario en la sesión de otro en dispositivos compartidos). **Residual conocido:** `notify.js` todavía usa el `session_id` del body (severidad baja: dispara emails internos a Martín, no exfiltra datos al atacante; el path de cron no tiene cookie) — pendiente de endurecer.
